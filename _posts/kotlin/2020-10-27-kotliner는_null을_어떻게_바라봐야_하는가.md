...
str2는 null일수도 있는 String타입이므로 null을 할당해도 에러가 나지 않는다. str2은 null일 수도 있기 때문에, 사용하는 측에서는 null check가 강제되어야만 한다. 3번째 라인처럼 null check를 하지 않으면 당연히 컴파일 에러가 나야 안전한 타입 시스템이다.
