...
더 이상 `Button`에서 text를 변경할 때, 상속받은 `text` 속성에 "hello"를 입력하지 않아야 합니다. 대신 `Button`이 `TextView`를 가지고 있게 해서, `TextView`가 랜더링 하도록 위임하면 됩니다. 그리고 이게 바로 Compose가 설계된 방식이고, 여러분이 Jetpack Compose로 UI를 그릴 때 사용할 방식입니다.
